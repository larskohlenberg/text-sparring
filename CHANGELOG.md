...
### Fixed
- **VollAutoSubagent-Modus hinzugefügt — behebt hängenden INIT-Agent bei Subagent-Konfiguration:** Wenn beide Agenten Claude sind und `Ausführungsmodus: VollAutoSubagent` gewählt wird, orchestriert der INIT-Agent alle Schritte aller Runden vollautomatisch als Subagenten-Dispatch-Loop — ohne Handover-Prompt, ohne Wait-Loop, ohne zweite Session. Bislang hatte `Ausführungsmodus: Subagent` nur die Ausführung des eigenen Schritts als Subagent gesteuert; nach Schritt 1 endete der INIT-Agent trotzdem im manuellen Handover + Wait-Loop. Mit dem neuen Modus ist klar getrennt: `Subagent` = Zwei-Session-Architektur mit Watch-Loop, `VollAutoSubagent` = Ein-Session-Orchestrierung bis zum Finalartefakt. Der neue Modus ist in Interview Q5 als erste Option gelistet.
...
- **CHALLENGE.md-Template auf Step-relevante Regeln reduziert.** Orchestrierungsdetails wie Zweck, Ausführungsmodus, Exit-Bedingungen, Silent-Wait-Regeln und INIT-only Auto-Detection liegen jetzt ausschließlich in `SKILL.md`, `state.md` und `watch_loop.sh`. Das generierte `CHALLENGE.md` bleibt damit kleiner und enthält nur noch das Regelwerk, das Step-Worker tatsächlich für These, Antithese und Synthese brauchen.
