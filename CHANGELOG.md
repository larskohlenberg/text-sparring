...
### Added
- **EXTEND-Modus (Sparring um Runden verlängern):** Neuer dritter Modus neben INIT und JOIN. Trigger-Phrasen wie *"Verlängere Sparring readme-v1 um 3 Runden"*, *"erweitere auf 8 Runden"*, *"extend by 3"* hängen die genannte Rundenzahl an ein bestehendes Sparring an — bis max. 10 Runden gesamt. Funktioniert sowohl für abgeschlossene Sparrings (häufigster Fall: nach manueller Sichtung des Finalartefakts ist mehr Schärfung gewünscht) als auch für laufende Sparrings (Plan wird nachträglich erweitert). Bei abgeschlossenen Sparrings wird das alte `FINAL_ARTIFACT` als Snapshot zu `FINAL_ARTIFACT_after_round_N` archiviert, die letzte Synthese wird zum Artefakt der neuen Runde, und der Wait-Loop wird neu gestartet mit Handover-Prompt für den anderen Agenten.
